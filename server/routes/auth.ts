...
import { LoginRequest, LoginResponse, RegisterRequest, RegisterResponse } from "@shared/customs-types";
...
export const registerHandler: RequestHandler = async (req, res) => {
  try {
    ...
    try {
      ...
      `;
      const userResult = await pool.query(userInsertQuery, [email, hashedPassword, full_name, phone]);
...
export const getCurrentUserHandler: RequestHandler = async (req, res) => {
  try {
    ...
    const userQuery = 'SELECT id, email, full_name, phone, user_type, is_active, created_at, updated_at FROM users WHERE id = $1';
